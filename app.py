...
# --- 2. GİRİŞ SİSTEMİ (SESSION STATE) ---
if 'giris_yapildi' not in st.session_state:
    st.session_state['giris_yapildi'] = False

def giris_ekrani():
    st.markdown("<div style='text-align: center; padding-top: 50px;'>", unsafe_allow_html=True)
    st.markdown("<h1>🔒 Güvenli Giriş</h1>", unsafe_allow_html=True)
    st.markdown("<p>Kariyer Merkezi Yönetim Paneli</p>", unsafe_allow_html=True)
...
if not st.session_state['giris_yapildi']:
    ...
    # --- BURADAN AŞAĞISI GİRİŞ YAPILDIKTAN SONRA GÖRÜNÜR ---
    
    # Yan Menü - Çıkış Butonu
    with st.sidebar:
        st.info(f"👤 Yönetici Girişi Yapıldı")
        if st.button("🚪 Çıkış Yap"):
            cikis_yap()
        st.markdown("---")

    # --- DOSYA VE VERİTABANI İŞLEMLERİ ---
    ...
    # --- SEKMELER VE MADDELER ---
    tab1, tab2, tab3 = st.tabs(["🟦  PLANLA", "🟨  KONTROL ET", "🟩  ÖNLEM AL"])

    # --- TAB 1: PLANLA ---
    with tab1:
        st.header("P - PLANLA (Planlama Aşaması)")
        ...
        with col2:
            ...
            # İsteğin üzerine buraya eklendi (Çakışma olmasın diye diğer taraftan silindi)
    ...
    # --- TAB 2: KONTROL ET ---
    with tab2:
        st.header("K - KONTROL ET (Değerlendirme ve İzleme)")
    ...
    # --- TAB 3: ÖNLEM AL ---
    with tab3:
        st.header("Ö - ÖNLEM AL (İyileştirme ve Sonraki Süreç)")
